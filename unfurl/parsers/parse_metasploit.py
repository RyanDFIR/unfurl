...
def run(unfurl, node):
    ...
    if node.data_type == 'url.path':
        # Metasploit's shellcode URI is the *entire* path: exactly four characters.
        # Keep this anchored to the whole path rather than testing each segment of a
        # longer one -- the checksum is only 8 bits and 30 of its 256 values map to a
        # platform, so an arbitrary 4-character segment would match ~12% of the time.
        shellcode_uri = node.value.strip('/')

        if len(shellcode_uri) == 4 and '/' not in shellcode_uri:
            metasploit_checksum = metasploit_checksums.get(sum(map(ord, shellcode_uri)) % 0x100)

            if metasploit_checksum:
                unfurl.add_to_queue(
                    ...
                    hover='Shellcode URLs generated by Metasploit and Cobalt Strike have a path <br>'
                          ...
                          '[<a href="https://isc.sans.edu/forums/diary/Finding+Metasploit+Cobalt+Strike+URLs/27204/">'
                          'ref</a>]',
    ...
    # Checked per path segment, not against the whole path: the UUID is not always the
    # only thing in the path (a redirector may prefix one), and a segment never contains
    # a "/", so no slash trimming is needed. Unlike the 4-character checksum above, this
    # carries its own strong validation -- the decoded platform, architecture, and
    # timestamp must all be valid -- so testing more candidates is cheap.
    if node.data_type == 'url.path.segment' and len(node.value) >= 23:

        # URL path segment should be base64 encoded
        if not (utils.digits_re.search(node.value) and utils.letters_re.search(node.value)
                and utils.urlsafe_b64_re.fullmatch(node.value)):
            return

        decoded = base64.urlsafe_b64decode(node.value[0:22] + '==')
