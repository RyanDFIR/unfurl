...
class TestSafeLinks(unittest.TestCase):
    ...
    def test_safelinks_decodes_destination(self):
        ...
        test.add_to_queue(
            ...
            value='https://nam04.safelinks.protection.outlook.com/?'
                  ...
                  'data=05%7C02%7Calice%40contoso.com%7Cabc-sender-guid%7C'
                  'def-tenant-guid%7C0%7C0%7C638400000000000000%7CUnknown%7C'
                  'TWFpbGZsb3d8eyJWIjoiMC4wLjAwMDAifQ%3D%3D&'
        ...
        self.assertTrue(
            any('Wrapped destination URL' in label for label in labels),
        ...
        self.assertTrue(any('Signature Data' in label for label in labels),
    ...
    def test_safelinks_double_encoded_data(self):
        ...
        self.assertTrue(any('Message GUID (Field 3): abc-sender' in label for label in labels),
                        f'double-encoded data not split; labels={labels}')
        self.assertTrue(any('Tenant GUID (Field 4): def-tenant' in label for label in labels),
                        f'double-encoded data not split; labels={labels}')
    ...
    def test_safelinks_only_fires_on_safelinks_domain(self):
        ...
        labels = self._node_labels(test)
        self.assertFalse(any('Signature Data' in label for label in labels),
                         'SafeLinks parser fired on non-SafeLinks domain')
        self.assertFalse(any('Recipient:' in label for label in labels),
                         'SafeLinks parser fired on non-SafeLinks domain')
